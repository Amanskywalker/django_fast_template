...
class GenerateUserOTP(APIView):
    ...
    Generate a 6 digit otp for the user to authenticate itself
        ...
        try:
            ...
            if phone is not None and (len(phone) >= 10) and phone.isdigit():
                ...
                try:
                    ...
                    # Register the user on Coruscant, or get the detials if user already registered
                    coruscant_user_detail, _ = CustomerProfile.objects.get_or_create(
                        phone=phone
                    )

                    # Now register the user on Tatooine
                    user_detail = get_user_model()(
                        phone=phone, core_id=coruscant_user_detail.id
                    )
                    ...
                    if settings.DEV or settings.DEBUG:
                        ...
                        TelegramService.send_message(
                            TelegramService, text=f"OTP Requested : {phone} : {otp}"
                        )
                    else:
                        ...
                        logger.info(f"Sent OTP : {sent_otp}")

                return Response({"success": True}, status=202)

            return Response(
                {"success": False, "error": f"phone number is needed"}, status=400
            )
